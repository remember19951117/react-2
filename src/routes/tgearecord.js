...
export default class Record extends Component {
    ...
    componentDidMount() {
        const goodsid = this.props.location.search.replace("?cmid=", "");
        ...
        fetchs.creat_Token(fetchs.APIHost + "/candy/log", fetchs.getAuth("/candy/log"), JSON.stringify(data)).then(response => response.json())
            .then(json => {
                            ...
                            tgrecodedata.length > 0 ? this.state.data.items.map(function (result) {
                                return <div className={styles.DItem}>
                                    <div className={styles.DItemLeft}>
                                        ...
                                        <p><span className={styles.DInfoName}>产生时间</span><span>{result.create_time}</span></p>
                                    </div>
