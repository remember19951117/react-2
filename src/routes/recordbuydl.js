...
export default class Record extends Component {
    ...
    componentDidMount() {
        var buydata = {
            page: 1,
            size: 10,
            //agent_uid: store.get("userinfo").uid,
        }
        //console.log("post",buydata)
        fetchs.creat_Token(fetchs.APIHost + "/agent/paylog", fetchs.getAuth("/agent/paylog"), JSON.stringify(buydata)).then(response => response.json())
            .then(json => {
               // //console.log("充值记录", json)
                this.setState({ jiludata: json.data})
        });
    }
         inputPwd = (value) => {
        ...
        return (
            <div>
                ...
                <div className={styles.main}>
                    ...
                    <div className={styles.ChgBtnBox} >
                        <div className={this.state.selectBtn === 1 ? styles.selectBtn : ""}>充值记录</div>
                            ...
                            record.length > 0 ? record.map(function (result, index) {
                                return <div className={styles.DItem} onClick={() => that.showInfo(result)} key={index}>
                                    <div className={styles.DItemLeft}>
                                        ...
                                        <p>
                                            <span className={styles.DInfoName}>转账用户</span>
                                            <span>{result.user.tel}</span>
                                        </p>
                                        <p><span className={styles.DInfoName}>交易状态</span><span>实时到账</span></p>
            ...
            />}>打款凭证</Item> */}
                                    ...
                                    <div className={ModalStyles.PlateBodyBBtnDiv}>
                                         <div className={ModalStyles.PlateBodyBBtn} onClick={this.tap.bind(this)}>
                                                    留言
                                            </div>
